...
void get_sum_matrix(matrix *matrix1, matrix *matrix2, matrix *m_result)
{
    ...
    {
        char *p_on_1st_element = (char *)matrix1->element;
        char *p_on_2nd_element = (char *)matrix2->element;
        char *p_on_result = (char *)m_result->element;
...
{
    char *new_elemnt_ptr = (char *)element_ptr;
    ...
    if (matrix != NULL)
    {
        ...
        if (size_element == 4)
        {
            for (int i = 0; i < (matrix->columns); i++)
            {
                ...
                get_increment_element(matrix, p_on_1st_line);
            }
        }
        else
        {
            ...
            for (int i = 0; i < (matrix->columns); i++)
            {
                *first_string_ptr = *(double *)coefficient * *second_string_ptr;
                get_increment_element(matrix, first_string_ptr);
            }
        }
    }
}